...
Info* largestBST(Node* root)
{
    ...
    int currMin = std::min(root->data,std::min(leftInfo->min, rightInfo->min));
    int currMax = std::max(root->data, std::max(leftInfo->max, rightInfo->max));
    int currSz = leftInfo->sz + rightInfo->sz + 1;

    if (leftInfo->isBST && rightInfo->isBST && root->data > leftInfo->max && root->data < rightInfo->min)
        {

    maxSize = std::max(maxSize, currSz);
